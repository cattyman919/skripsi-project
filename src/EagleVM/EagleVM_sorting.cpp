...
void print_graphviz(const std::vector<ir::block_ptr> &blocks,
  ...
  for (const auto &block : blocks) {
    ...
    for (const auto &inst : *block)
      insts_nodes << std::format("<TR><TD ALIGN=\"LEFT\">{}</TD></TR>",
                                 inst->to_string());
    ...
    for (const auto &branch : branches) {
      ...
      if (std::holds_alternative<ir::block_ptr>(branch))
        target_id =
            std::format("0x{:x}", std::get<ir::block_ptr>(branch)->block_id);
      else
...
int main(int argc, char *argv[]) {
  auto executable = argc > 1 ? argv[1] : "sorting.exe";
  ...
  if (parsing_type) {
    ...
    const win::data_directory_t export_data_dir =
        parser->get_nt_headers()
            ->optional_header.data_directories.export_directory;
    win::export_directory_t *export_dir =
        parser->rva_to_ptr<win::export_directory_t>(export_data_dir.rva);
    for (auto i = 0; i < export_dir->num_names; i++) {
      uint32_t *address_of_names =
          parser->rva_to_ptr<uint32_t>(export_dir->rva_names);
      std::string import_name =
          std::string(parser->rva_to_ptr<char>(address_of_names[i]));

      uint16_t *address_of_name_ordinals =
          parser->rva_to_ptr<uint16_t>(export_dir->rva_name_ordinals);
      uint16_t name_ordinals = address_of_name_ordinals[i];

      uint32_t *address_of_functions =
          parser->rva_to_ptr<uint32_t>(export_dir->rva_functions);
    ...
    const auto exception_data_dir =
        parser->get_nt_headers()
            ->optional_header.data_directories.exception_directory;
    const auto exception_data_dir_count =
        exception_data_dir.size / sizeof(win::runtime_function_t);

    win::runtime_function_t *exception_dir =
        parser->rva_to_ptr<win::runtime_function_t>(exception_data_dir.rva);
    for (int i = 0; i < exception_data_dir_count; i++) {
      auto runtime_func = &exception_dir[i];
      auto res = std::ranges::find_if(marked_function, [&](auto &a) {
        return a.first == runtime_func->rva_begin;
      });
    ...
    for (auto &[start, end] : marked_function) {
      vm_iat_calls.emplace_back(pe::stub_import::vm_begin,
                                parser->rva_to_fo(start));
      vm_iat_calls.emplace_back(pe::stub_import::vm_end,
                                parser->rva_to_fo(end));
    }
  } else {
    std::printf("\n[>] image imports\n");
    std::printf("%3s %-20s %-s\n", "", "source", "import");

    const win::data_directory_t import_data_dir =
        parser->get_nt_headers()
            ->optional_header.data_directories.import_directory;
    win::import_directory_t *import_sec =
        parser->rva_to_ptr<win::import_directory_t>(import_data_dir.rva);

    uint64_t rva_begin = 0, rva_end = 0;
    while (import_sec->rva_name) {
      const char *module_name = parser->rva_to_ptr<char>(import_sec->rva_name);
      win::image_thunk_data_x64_t *thunk =
          parser->rva_to_ptr<win::image_thunk_data_x64_t>(
              import_sec->rva_original_first_thunk);

      int i = 0;
      while (thunk->address) {
        if (!(thunk->ordinal & IMAGE_ORDINAL_FLAG)) {
          win::image_named_import_t *import =
              parser->rva_to_ptr<win::image_named_import_t>(thunk->address);
    ...
    for (auto section : parser->get_nt_headers()->sections()) {
      for (auto i = section.ptr_raw_data;
           i < section.ptr_raw_data + section.size_raw_data; i++) {
        if (*parser->raw_to_ptr(i) == 0xFF &&
            *parser->raw_to_ptr(i + 1) == 0x15) {
  ...
  for (int c = 0; c < vm_iat_calls.size(); c += 2) // i1 = vm_begin, i2 = vm_end
  {
    ...
    uint32_t rva_inst_begin =
        parser->fo_to_rva(vm_iat_calls[c].second) + call_size_64;
    ...
    dasm::segment_dasm_ptr dasm = std::make_shared<dasm::segment_dasm>(
        rva_inst_begin, pinst_begin, rva_inst_end - rva_inst_begin);
    ...
    for (auto &block : dasm->get_blocks()) {
      ...
      auto bitfield_to_bitstring = [](const uint64_t value,
                                      const auto sig_bits) -> std::string {
        std::string result;
      ...
      for (size_t idx = 0; auto &inst : block->decoded_insts) {
        ...
        printf("\tin: \n");
        for (int k = ZYDIS_REGISTER_RAX; k <= ZYDIS_REGISTER_R15; k++)
          if (auto res = block_liveness[idx].first.get_gpr64(
                  static_cast<codec::reg>(k)))
            printf("\t\t%s:%s\n", reg_to_string(static_cast<codec::reg>(k)),
                   bitfield_to_bitstring(res, 8).c_str());
        ...
        printf("\tout: \n");
        for (int k = ZYDIS_REGISTER_RAX; k <= ZYDIS_REGISTER_R15; k++)
          if (auto res = block_liveness[idx].second.get_gpr64(
                  static_cast<codec::reg>(k)))
            printf("\t\t%s:%s\n", reg_to_string(static_cast<codec::reg>(k)),
                   bitfield_to_bitstring(res, 8).c_str());
    ...
    std::shared_ptr ir_trans =
        std::make_shared<ir::ir_translator>(dasm, &seg_live);
    ...
    for (const auto &preopt_block : preopt)
      if (preopt_block->original_block ==
          dasm->get_block(rva_inst_begin, false))
        entry_block = preopt_block;

    VM_ASSERT(entry_block != nullptr,
              "could not find matching preopt block for entry block");

    // if we want, we can do a little optimzation which will rewrite the preopt
    // blocks, or we could simply ir_trans.flatten()
    std::unordered_map<ir::preopt_block_ptr, ir::block_ptr> block_tracker = {
        {entry_block, nullptr}};
    std::vector<ir::flat_block_vmid> vm_blocks =
        ir_trans->optimize(block_vm_ids, block_tracker, {entry_block});

    std::unordered_map<uint32_t, std::vector<ir::block_ptr>> vm_id_map;
    for (auto &[block, vmid] : vm_blocks) {
      // while setting up the map we also run the obfuscation pass for handler
      // merging
      vm_id_map[vmid].append_range(block);
    }

    for (auto &blocks : vm_id_map | std::views::values) {
      std::vector<ir::block_ptr> handler_blocks =
      ...
      //    {
      //        if (block->at(i)->get_command_type() ==
      //        ir::command_type::vm_call)
      //        {
    ...
    virt::eg::settings_ptr machine_settings =
        std::make_shared<virt::eg::settings>();
    ...
    for (const auto &blocks : vm_id_map | std::views::values) {
      ...
      virt::eg::machine_ptr machine =
          virt::eg::machine::create(machine_settings);
      ...
      for (auto i = 0; i < blocks.size(); i++) {
        ...
        asmb::code_container_ptr result_container =
            machine->lift_block(translated_block);
      ...
      // build handlers
      std::vector<asmb::code_container_ptr> handler_containers =
          machine->create_handlers();
    ...
    va_nop.emplace_back(parser->fo_to_rva(vm_iat_calls[c + 1].second),
                        call_size_64);

    // add vmenter for root block
    va_enters.emplace_back(parser->fo_to_rva(vm_iat_calls[c].second),
                           entry_point);
  }

  std::printf("\n");

  win::section_header_t *last_section = parser->get_nt_headers()->get_section(
      parser->get_nt_headers()->sections().count - 1);

  auto &[code_section, code_section_bytes] = generator.add_section(".hihihi");
  code_section.ptr_raw_data =
      last_section->ptr_raw_data + last_section->size_raw_data;
  code_section.size_raw_data = 0;
  code_section.virtual_address = generator.align_section(
  ...
  codec::encoded_vec vm_code_bytes =
      vm_section.compile_section(code_section.virtual_address);
  ...
  for (auto &[enter_va, enter_location] : va_enters) {
    codec::enc::req jump_request = encode(
        codec::m_jmp, ZIMMS(enter_location->get_address() - enter_va - 5));
  ...
  // custom entry point
  {
    ...
    packer_section.ptr_raw_data =
        last_section->ptr_raw_data + last_section->size_raw_data;
    packer_section.size_raw_data = 0;
    packer_section.virtual_address = generator.align_section(
        last_section->virtual_address + last_section->virtual_size);
    ...
    codec::encoded_vec packer_code_bytes =
        packer_sm.compile_section(code_section.virtual_address);
    auto [packer_pdb_offset, size] =
        pe::pe_packer::insert_pdb(packer_code_bytes);

    packer_section.size_raw_data =
        generator.align_file(packer_code_bytes.size());
    packer_section.virtual_size =
        generator.align_section(packer_code_bytes.size());
    packer_bytes += packer_code_bytes;

    generator.add_custom_pdb(packer_section.virtual_address + packer_pdb_offset,
  ...
  std::string target_file_name =
      path.stem().string() + "_protected" + path.extension().string();
