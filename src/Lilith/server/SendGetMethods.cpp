...
bool Server::recvall(int ID, char *data, int totalbytes) {
  ...
  {
    int RetnCheck =
        recv(connections[ID]->socket, data, totalbytes - bytesreceived,
...
bool Server::sendall(int ID, char *data, int totalbytes) {
  ...
  {
    int RetnCheck =
        send(connections[ID]->socket, data + bytessent, totalbytes - bytessent,
...
bool Server::Sendint32_t(int ID, int32_t _int32_t) {
  ...
  if (!sendall(ID, (char *)&_int32_t,
               sizeof(int32_t))) // Try to send long (4 byte int)... If int
                                 // fails to send
...
bool Server::Getint32_t(int ID, int32_t &_int32_t) {
  if (!recvall(ID, (char *)&_int32_t,
               sizeof(int32_t))) // Try to receive long (4 byte int)... If int
                                 // fails to be recv'd
...
bool Server::SendPacketType(int ID, PacketType _packettype) {
  if (!Sendint32_t(ID, (int32_t)_packettype)) // Try to send packet type... If
                                              // packet type fails to send
...
bool Server::GetPacketType(int ID, PacketType &_packettype) {
  int packettype;
  if (!Getint32_t(ID, packettype)) // Try to receive packet type... If packet
                                   // type fails to be recv'd
    return false; // Return false: packet type not successfully received
  _packettype = (PacketType)packettype;
  return true; // Return true if we were successful in retrieving the packet
               // type
}

void Server::SendString(int ID, const std::string &_string,
...
bool Server::GetString(int ID, std::string &_string) {
  ...
  if (!Getint32_t(ID, bufferlength)) // Get length of buffer and store it in
