...
public:
  ...
  bool ActiveConnection; // True if connection is active, false if inactive(due
                         // to a disconnect)
  SOCKET socket;
  // file transfer data
  FileTransferData file; // Object that contains information about our file that
                         // is being sent to the client from this server
...
public:
  ...
  void HandleInput();
  static Server
      *serverptr; // Serverptr is necessary so the static ClientHandler method
                  // can access the server instance/functions.
...
private:
  ...
  void DisconnectClient(int ID); // Called to properly disconnect and clean up a
                                 // client (if possible)
private:
  std::vector<std::shared_ptr<Connection>> connections;
  std::mutex connectionMgr_mutex; // mutex for managing connections (used when a
                                  // client disconnects)
  int UnusedConnections =
      0; // # of Inactive Connection Objects that can be reused
